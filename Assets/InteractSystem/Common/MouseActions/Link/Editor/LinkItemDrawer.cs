...
namespace InteractSystem.Drawer
{
    ...
    public class LinkItemDrawer : ActionItemDrawer
    {
        ...
        protected override void OnDrawPropertys()
        {
            base.OnDrawPropertys();
            ...
            if(selected == 0)
            {
                using (var hor = new EditorGUILayout.HorizontalScope())
                {
                    ...
                    if (GUILayout.Button("坐标坍缩", EditorStyles.miniButtonRight, GUILayout.Width(100)))
                    {
        ...
        {
            if (property.propertyPath == "_childNodes")
            {
                DrawLinkPortInfos();
            }
            else
